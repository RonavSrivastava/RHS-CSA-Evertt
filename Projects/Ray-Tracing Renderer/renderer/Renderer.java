    ...
    public void render(WritableRaster renderTarget, World world, Camera camera) {
        for (int y = 0; y < renderTarget.getHeight(); y++) {
            for (int x = 0; x < renderTarget.getWidth(); x++) {
                ...
                renderTarget.setPixel(x, y, new int[] { rayCast(world, camera.getPos(), camera.forward().add(new Vec3(x, y, 10))).color.getRed(), rayCast(world, camera.getPos(), camera.forward().add(new Vec3(x, y, 10))).color.getGreen(), rayCast(world, camera.getPos(), camera.forward().add(new Vec3(x, y, 10))).color.getBlue(), 255 }); // R, G, B, Alpha (0 to 255 values)
            }
        }
    }
}
